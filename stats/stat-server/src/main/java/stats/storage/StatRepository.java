...
public interface StatRepository extends JpaRepository<Stats, Long> {
    ...
    @Query("""
        SELECT new dto.StatDto(s.app, s.uri, COUNT(DISTINCT s.ip))
        FROM Stats s
    ...
    """)
    List<StatDto> getUniqueStat(@Param("start") LocalDateTime start,
    ...
    @Query("""
        SELECT new dto.StatDto(s.app, s.uri, COUNT(s.ip))
        FROM Stats s
        WHERE s.timestamp BETWEEN :start AND :end
        AND (:uris IS NULL OR s.uri IN :uris)
        GROUP BY s.app, s.uri
        ORDER BY COUNT(s.ip) DESC
    """)
    List<StatDto> getNonUniqueStat(@Param("start") LocalDateTime start,
                                   @Param("end") LocalDateTime end,
                                   @Param("uris") List<String> uris);
}
